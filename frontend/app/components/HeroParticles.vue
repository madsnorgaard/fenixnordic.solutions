...
interface Particle {
  ...
  isEmber: boolean
  pulsePhase: number
  colorBase: string  // pre-baked "r,g,b" so there is no per-frame lerp
  flashTimer: number
}
...
// Pre-allocated flat line-segment buffers, so no GC per frame
...
// Colour temperature: cold ember, then brand, then hot amber
...
onMounted(() => {
  if (!canvasRef.value || reduced()) return
...
onUnmounted(() => {
  ...
  canvasRef.value?.closest('.hero')?.removeEventListener('mouseleave', onMouseLeave)
})
</script>
